...
很多初学者在学 LangChain 时，会把注意力都放在“接哪个模型”“换哪个平台”“怎么配 API Key”上，但真正到了项目里，最容易让系统效果不稳定的，往往不是模型接入，而是**输入组织得不够清楚**。
...
这里的 `{name}` 就像 Prompt 模板里的占位符。你可以先把它理解成“先留坑，后填值”。
...
其中，`partial_variables` 对初学者尤其值得理解。它的作用是：
...
经验上更推荐初学者优先使用 `from_messages(...)`，因为可读性更好，也更符合官方文档和社区示例的主流写法。
...
MessagesPlaceholder（消息占位符）这是本章最重要的知识点之一，很多初学者一开始会觉得它抽象，但一旦理解，就会发现它几乎是多轮对话、记忆、历史上下文拼接的关键。
...
当 Prompt 还很短时，把它直接写在 Python 代码里问题不大；但只要你开始做真实项目，就会很快遇到几个问题：

- Prompt 越来越长，代码越来越乱
- 产品、运营、算法同学希望一起改 Prompt
- 需要保留多个版本做 A/B 测试
- 想把 Prompt 与代码逻辑分离
