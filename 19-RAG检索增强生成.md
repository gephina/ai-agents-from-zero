...
如果只是入门，你先记住前面的“两阶段”就够了；但从真实项目角度看，很多 RAG 系统会在“检索与生成”之间再补几步，让结果更稳。第 2 章里你已经在 Dify 里看到了 Top K、Rerank、混合检索这些参数，这里可以把它们和代码链路对上：
...
- 从掌握结果看，学完本章后，你至少应该：能用“**索引阶段**”和“**检索与生成阶段**”两段式描述 RAG 的标准流程；知道文档加载器、文本分割器、Embedding、向量库、检索器、Prompt、聊天模型在 RAG 里各自负责哪一步；能分清 **管道式 RAG** 和 **Agent 式 RAG** 的差别，并知道 RAG 质量是“文档、检索、生成、评估”共同决定的。
