...
前面几章，我们已经把「掌柜问数」的元数据知识库基本搭好了。系统现在知道有哪些表、有哪些字段、字段是什么意思、字段里有哪些真实取值，也知道业务指标如何定义。具体能力如下：
...
前面第 22 到 26 章已经系统讲过 LangGraph。本章不是重新讲一遍 API 手册，而是把这些概念放回「掌柜问数」项目里，做一次项目落地版复习。
...
项目对应文件路径：`shopkeeper-agent-backend/app/agent/state.py`
...
项目对应文件路径：`shopkeeper-agent-backend/app/agent/context.py`
...
项目对应文件路径：`shopkeeper-agent-backend/app/agent/nodes/extract_keywords.py`
...
项目对应文件路径：`shopkeeper-agent-backend/app/agent/graph.py`
