...
**学习建议：** 本章建议按 **“先理解为什么只靠自然语言输出不够 → 再看 Parser 的最小用法 → 再分清 Structured Output 和 Parser 的关系 → 最后再看 TypedDict / Pydantic / JSON Schema 的取舍”** 的顺序学习。对初学者来说，最容易混淆的不是某个 API，而是“解析器”和“结构化输出”到底是不是一回事，本章会把这条主线单独讲清楚。
...
- 不再单独手写 parser；
- 由 `with_structured_output(...)` 统一完成“约束输出 + 自动解析”；
- 适合现代模型支持原生结构化输出的场景。
...
> **说明**：很多入门教程会笼统说“`parse(result)` 用来解析模型结果”。从概念上这样理解没问题，但更准确地说，`parse(...)` 更偏向解析**文本字符串**，而链式开发中我们常直接使用 `parser.invoke(result)`。
...
很多同学会问：既然 `AIMessage.content` 也能直接拿文本，为什么还要 `StrOutputParser`？
...
但输出解析器并没有过时，它仍然很有价值，尤其是在：

- 使用不支持原生结构化输出的模型时；
- 需要把结果继续做解析/清洗时；
- 需要用 Pydantic 追加严格校验时；
- 想把输出解析作为 LCEL 链的一环统一管理时。
